...
exports.createEmployee = async (req, res) => {
  ...
  if (!req.body.name) {
    res.status(400).json({ message: "O nome não pode ser nulo!" });
  }
  if (!req.body.register) {
    res.status(400).json({ message: "O campo matrícula não pode ser nulo!" });
  }
  if (!req.body.password) {
    res.status(400).json({ message: "O campo senha não pode ser nulo!" });
  }
  ...
  try {
    ...
    return res.status(201).json({
      message: `Registro concluído, seu token é: ${tokenGenerator({
        id: register
...
exports.loginEmployee = async (req, res) => {
  try {
    ...
    const checkEmployee = await Employee.findOne({ register }).select(
      "password"
    );
    if (!checkEmployee) {
...
exports.getEmployeeById = async (req, res) => {
  try {
    ...
    const findEmployeeById = await Employee.findOne({
      register: employeeId,
    }).select(["-__v", "-_id"]);
    if (!findEmployeeById) {
...
exports.updateProduct = async (req, res) => {
  try {
    const productId = req.body.cod;
    if (!productId) {
      return res.status(500).json({ error: "Erro ao atualizar o produto, pois não foi fornecido um cod." });

    }
    const findProduct = await Product.findOne({ cod: productId })
    if (!findProduct) {
      return res.status(500).json({ error: "Produto não localizado." });
   ...
   return res.json({ message: `O produto ${findProduct.item}, foi alterado.` });
  } catch (error) {
...
exports.deleteProduct = async (req, res) => {
  try {
    ...
    res.json({
      message: `Produto ${deleteProduct.name} removido com sucesso.`,
    });
  } catch (error) {
